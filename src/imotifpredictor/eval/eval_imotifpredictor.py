...
def compute_aupr(
    ...
    This is the metric reported in the manuscript for genome-wide evaluation.
    """
...
def compute_pr_curve(
    ...
    """
    Return (precision, recall) arrays for PR curve plotting.
    """
    s = np.asarray(y_score, dtype=float)
    if np.isnan(s).any():
        s = np.nan_to_num(s, nan=nan_fill)
...
def evaluate_predictions_table(
    ...
    """
    Evaluate all candidate prediction columns in a table and rank them by AUPR.
